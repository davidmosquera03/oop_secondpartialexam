...
# This is a State Pattern Code
class NMEA_State:
    
# the Base State pattern with the methods all states have and will implement different
    def __init__(self, message: "Message") -> None:
        self.message = message # refers to the object of Class Message it will alter
...
class Waiting(NMEA_State): # A Concrete State where feed_byte is changed
    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
            return Header(self.message) # Returns the message, no addition done
        return self


class Header(NMEA_State):   # Another State
    def __init__(self, message: "Message") -> None:
        self.message = message
        self.message.reset() # Resets the message object

    def feed_byte(self, input: int) -> NMEA_State:
        if input == ord(b"$"):
            # Reset any accumulated bytes
            return Header(self.message)
        size = self.message.body_append(input) # Adds the input to the message
        if size == 5:
...
class Message: # The Class whose behaviour will be changed by alterations in internal states
...
class Reader:
    def __init__(self) -> None:
        ...
        # Sets an initial state for the message
